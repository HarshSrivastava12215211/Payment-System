...
public class KycService {
    ...
    public void updateKycStatus(String userId, KycStatus status) {
        Kyc kyc = repository.findByUserId(userId).orElseThrow(() -> new RuntimeException("KYC not found for user"));
        kyc.setStatus(status);
        repository.save(kyc);
    }

    public java.util.Optional<Kyc> getKycByUserId(String userId) {
